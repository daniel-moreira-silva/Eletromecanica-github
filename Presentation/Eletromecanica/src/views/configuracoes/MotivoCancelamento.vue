...
import MotivoCancelamentoService from "@/services/configuracoes/motivo-cancelamento-service.js";
...
const salvar = () => persistir((req) => motivoCancelamentoService.salvar(req));
const atualizar = () => persistir((req) => motivoCancelamentoService.atualizar(req));
...
async function alterarStatus({ item: itemGrid, valor }) {
  try {
    ...
    const resp = await motivoCancelamentoService.atualizarStatus(id, valor);
    loading.value = false;
    retorno.value = true;
...
async function listarItens() {
  ...
  const result = await motivoCancelamentoService.listar(filtro.value);
  ...
  if (result?.statusCode === 200) {
    ...
    mensagemRetorno.value = result?.data?.message || "Falha ao listar motivos de cancelamento.";
    sucesso.value = false;
    retorno.value = true;
  }
}
            ...
            <v-card-text class="pa-4">
              <div class="d-flex align-center pb-2">
                ...
                <span class="title black--text">
                  {{ inserindo ? "Inserir Motivo de Cancelamento" : "Editar Motivo de Cancelamento" }}
                </span>
                <v-spacer></v-spacer>
                <font-awesome-icon
                  icon="xmark"
              ...
              <v-form ref="formulario">
                <v-container class="pa-3 pt-6 container-modal">
                  <v-row>
                    <v-col cols="12" md="4" class="pa-1 mt-2">
                      <v-text-field
                        v-model="item.codigo"
                        label="Código"
                        clearable
                    ...
                    <v-col cols="12" md="8" class="pa-1 mt-2">
                      <v-text-field
