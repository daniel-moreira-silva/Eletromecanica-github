...
internal class SetorRepository(DbConnection connection) : ISetorRepository
{
    public async Task<Guid> AddAsync(Setor setor, IDbTransaction? transaction = null, CancellationToken cancellationToken = default)
    {
        ...
        return await connection.ExecuteScalarAsync<Guid>(
            new CommandDefinition(sql, setor, transaction, cancellationToken: cancellationToken));
    }

    public async Task<bool> UpdateAsync(Setor setor, IDbTransaction? transaction = null, CancellationToken cancellationToken = default)
    {
        await DbUtils.EnsureOpenAsync(connection, cancellationToken);
    ...
    public async Task<Setor?> GetByIdAsync(Guid id, IDbTransaction? transaction = null, CancellationToken cancellationToken = default)
    {
        await DbUtils.EnsureOpenAsync(connection, cancellationToken);
    ...
    public async Task<ListaPaginada<Setor>> PaginatedGetAsync(SetorFilter filter, IDbTransaction? transaction = null, CancellationToken cancellationToken = default)
    {
        await DbUtils.EnsureOpenAsync(connection, cancellationToken);
        ...
        var lista = await connection.QueryAsync<Setor>(
        ...
        return new ListaPaginada<Setor>
        {
            Lista = lista.AsList(),
            Paginas = paginas,
            TotalItens = total
    ...
    public async Task<bool> ValidateDescriptionIsDuplicatedAsync(string descricao, Guid? id = null, IDbTransaction? transaction = null, CancellationToken cancellationToken = default)
    {
        ...
        string sql = @"SELECT 1 FROM Setor WHERE UPPER(Descricao) = UPPER(@Descricao)";
    ...
    public async Task<List<Setor>> GetAllAsync(IDbTransaction? transaction = null, CancellationToken cancellationToken = default)
    {
        await DbUtils.EnsureOpenAsync(connection, cancellationToken);
